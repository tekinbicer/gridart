...
void myart(float* data, float* theta, float center,
           int num_projs, int num_rows, int num_cols, 
           int num_grid, float* recon) {
    ...
    for (q = 0; q < num_projs; q++) {
        ...
        for (m = 0; m < num_cols; m++) {
            ...
            // Note: The indices (ind_recon) and the corresponding 
            // weights (len_recon) are the same for all slices. So,
            ...
            for (k = 0; k < num_rows; k++) {
                ...
                io = m+(k*num_cols)+q*(num_rows*num_cols);
